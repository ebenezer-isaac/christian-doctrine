...
| 7 | Cultural overlay engine | Pipeline 3 read-and-assemble stage. Reads the Layer 4 verdict, Layer 6/6.5 tags, and Layer 7.1 historical sidecar files. Attaches "how each lineage reads this lexical pattern" plus "how the world around Scripture recorded the event" as diagnostic blocks. The server assembles the blocks and returns structured data; the calling model synthesizes. Never edits the Layer 4 verdict. |
...
| 9 | MCP server and client | Python MCP SDK (`pip install mcp`). 12 tools (specified in docs/MCP_TOOLS.md), including `historical_inspect` for deep-linking the Layer 7.1 sidecar. Streamable HTTP transport with progress tokens for the long-running `doctrinal_verdict` tool. The Flutter client surface is intentionally not built in v1; v1 ships as MCP-only, queryable from Claude Code or other MCP-native clients. |
...
A user question arrives at the MCP server. The router classifies intent (scripture / named_figure / comparative / general / doctrinal-verdict). The surface is 12 tools (docs/MCP_TOOLS.md): the lexical tools (`lexical_lookup`, `concordance_walk`, `cross_ref`, `variant_inspect`, `parallel_translation`, `versification_resolve`) touch the lexical store only; `cultural_overlay` and `debate_for_verse` reach the cultural store; `doctrinal_verdict` is the synthesis surface; and `evidence_inspect`, `historical_inspect`, and `license_audit` are filesystem deep-links over `evidence/`, `historical/`, and the merged license stack.

For doctrinal queries, `doctrinal_verdict` returns three diagnostic blocks side by side, never fused into a single index:

1. Resolve the proposition to a `questions.json` entry, then read the locked `evidence/<id>.json` from Pipeline 2. This is the **lexical verdict**, and it is authoritative.
2. Retrieve **cultural overlay** chunks from the cultural store (`cult_col`), filtered by tradition and by the doctrine slug of the matched `questions.json` entry. This block is diagnostic and never settles the verdict.
3. Read the **historical attestation** sidecar from `historical/<id>.json` (the curated Pipeline 4 output; the underlying extra-biblical source chunks it draws on live in the historical store `hist_col`). It is sourced authoritatively from the Pipeline 4 file and NEVER re-derived at query time. Most of the 231 questions carry `attestation_present: false` (an empty block); 27 carry witnesses. This block is diagnostic too.

The server returns these three blocks as structured data and does the synthesis of none of them: it calls no LLM. The calling model (the MCP client) reads the blocks and reasons over them. The handler is self-contained and pure: `doctrinal_verdict.handle(payload, *, evidence_dir=None, historical_dir=None, cultural_chunks=None)`. It reads the locked evidence (authoritative), formats the cultural chunks the tool retrieved live (diagnostic), and reads the historical sidecar (diagnostic), assembling each as a separate block. The store connections live on a FastMCP `lifespan` (`cd_mcp/runtime.py`), opened once at startup and reached by each tool through the injected `Context`; the tool's `register()` retrieves the cultural chunks live and passes them into `handle`. There is no synthesis subagent, no `synthesis_fn`, and no dispatch. `python -m cd_mcp.server` is fully functional with no external wiring; `build_server()` takes no injector arguments.

Two authority rules bind the surface. First, **verdict fidelity by construction**: `result.verdict` is the stored `evidence[<id>].verdict.affirms`, read straight from the file. Because it is read and never re-derived at query time, it cannot drift; there is nothing to re-check. Second, **attestation authority**: the historical block is read verbatim from the sidecar, validated against `HistoricalAttestation`, and a malformed file aborts the response with `historical_corrupt`.

The query-time air-gap holds as strictly as the ingest-time one: the lexical tools never read the cultural or historical stores, and `doctrinal_verdict` reads all three as separate blocks with separate license stacks, never a single fused index. The output envelope segregates the three with separate citation blocks. The license stack is folded into the envelope (lexical plus cultural plus historical `sources_used`) so the calling agent never accidentally bulk-exports NC content (most notably ETCBC/dss for DSS chunks): a non-redistributable witness flips `response_safe_to_share` under `public-share` and `export`.
