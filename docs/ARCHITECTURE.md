...
The three are assembled by a synthesis subagent. `doctrinal_verdict.handle(synthesis_fn=...)` is a pure transform: the actual dispatch is injected through `make_synthesis_fn` in `bd_mcp/synthesis.py`, which mirrors the Pipeline 2 and Pipeline 4 injected-`dispatch_fn` pattern exactly. There is no programmatic Anthropic API; the orchestrator provides the dispatch bridge at runtime and a deterministic mock in tests. The subagent reads the locked evidence (authoritative), the retrieved cultural chunks (diagnostic), and the locked historical sidecar (diagnostic), each as a separate block. It may reference the witnesses in prose but does not author the historical block: the handler attaches that straight from the validated sidecar after synthesis returns, so the subagent cannot alter or invent attestation.

Two fidelity rules bind the synthesis surface. First, **verdict fidelity**: `result.verdict` must equal `evidence[<id>].verdict.affirms`; the handler asserts the equality and returns `verdict_fidelity_violation` on mismatch. Re-deriving the verdict at query time is forbidden. Second, **attestation authority**: the historical block is read verbatim from the sidecar, validated against `HistoricalAttestation`, and a malformed file aborts the response with `historical_corrupt`.

The query-time air-gap holds as strictly as the ingest-time one: the lexical tools never read the cultural or historical stores, and only the synthesis stage reads all three, as separate blocks with separate license stacks, never a single fused index. The output envelope segregates the three with separate citation blocks. The license stack is folded into the envelope (lexical plus cultural plus historical `sources_used`) so the calling agent never accidentally bulk-exports NC content (most notably ETCBC/dss for DSS chunks): a non-redistributable witness flips `response_safe_to_share` under `public-share` and `export`.
